...
class HorizontalLoginForm extends React.Component {
  ...
  componentWillReceiveProps = (props) => {
    // console.log('props: ', props.card[0])
    this.setState({name: props.card[0].title, number: props.card[0].number, date: props.card[0].expiryDate })
  }
  ...
  render() {
    ...
    return (
      <div>
       <Button style={{marginTop: 10}} type="primary" ghost onClick={() => {this.showModal()}}>Dados do cartão</Button>
            <Modal
            ...
            >
              {getFieldDecorator('cardname', {
                ...
                <Input defaultValue={this.state.name}  prefix={<Icon type="user" style={{ color: 'rgba(0,0,0,.25)', width: "100%" }}/>} onChange={this.onNameChange}  placeholder="Nome do cartão" />
              )}
            </Form.Item>
...
const ModalCreditCard = Form.create({ name: 'horizontal_login' })(HorizontalLoginForm);
